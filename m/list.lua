...
local table_key_of = require("balm/m/table").key_of
...
--- ): Table
function m.crawford_base32_le_rolling_encode_table(...)
  local items = {...}
  local len = #items
  local len1 = len + 1
  local i = 1
...
function m.reduce(list, acc, fun)
  local should_break
...
function m.get_next(list, current)
  ...
  if current then
    local index = table_key_of(list, current)
    if index then
...
function m.sort_by(list, callback)
  if next(list) then
    local size = #list
    local a
    local b
...
function m.filter(list, callback)
  local result = {}
  local i = 0
...
function m.reject(list, callback)
  local result = {}
  local i = 0
...
--- ): (value: V | nil, index: K | nil)
function m.bsearch_by(t, predicate)
  local len = #t
...
local list_bsearch_by = assert(m.bsearch_by)
--- @since "1.32.0"
--- @spec list_bsearch_by(
---   t: Table<K, V>,
---   v: Any,
--- ): (value: V | nil, index: K | nil)
function m.bsearch(t, a)
