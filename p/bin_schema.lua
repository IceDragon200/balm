local List = lily_core:require("util/list")
local Map = lily_core:require("util/map")
local ByteBuf = lily_core:require("util/byte_buf")
local ArrayType = lily_core:require("util/bin_types/array")
local MapType = lily_core:require("util/bin_types/map")
local ScalarTypes = lily_core:require("util/bin_types/scalars")

local BinSchema = lily.Object:extends("BinSchema")
...
function ic:initialize(definition)
  ...
  self.definition = List.map(definition, function (element)
...
function ic:write(file, data)
  return List.reduce(self.definition, 0, function (block, all_bytes_written)
    if block.type == 0 then
...
function ic:read(file, target)
  target = target or {}
  return target, List.reduce(self.definition, 0, function (block, all_bytes_read)
    if block.type == 0 then
