...
local m = require("balm/s/weighted_list")

local case = Luna:new("balm.s.WeightedList")
...
case:describe(":new/0", function (t2)
  t2:test("can create a new WeightedList", function (t3)
    ...
    t3:assert_eq(list:total_weight(), 0)
  end)
end)
...
case:describe("#push/2", function (t2)
  t2:test("can push a new element unto weighted list", function (t3)
    ...
    t2:assert_eq(list:size(), 1)
    t2:assert_eq(list:total_weight(), 4)

    list:push("Item 2", 8)

    t2:assert_eq(list:size(), 2)
    t2:assert_eq(list:total_weight(), 12)

    list:push("Item 3", 7)

    t2:assert_eq(list:size(), 3)
    t2:assert_eq(list:total_weight(), 19)
  end)
end)
